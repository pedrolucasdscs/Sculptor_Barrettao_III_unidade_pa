    ...
    for (int i=0;i<_nx;i++){
      ...
      for (int j=0;j<_ny;j++)
      {
        ...
        for (int k=0; k<_nz;k++)
        {
         ...
         this->v[i][j][k].a = 0.0;              // Transparencia
...
void Sculptor::putVoxel(int x,int y,int z)               // O PutVoxel torna o show verdadeiro
{
    ...
    void Sculptor::cutVoxel(int x,int y,int z)           // O CutVoxel torna o show falso
 {
  ...
  void Sculptor::writeOFF(const char* filename)
{
  ...
  fout << std:: fixed;                     // Mostra um numero real enquanto fixo
  fout << std:: setprecision(1);          // Declara duas casas decimais apos a virgula
    int active = 0;                      // Faz a contagem dos voxels ativos
    int countActives = 0;                // Faz a contagem das faces e vertices 
  ...
  for(int i = 0;i < this->nx;i++)
  {
    for(int j = 0;j < this->ny;j++)
    {
      for(int k = 0;k < this->nz;k++)
      {
        if(this->v[i][j][k].show)
        {
          // O endl declara o fim da linha para a maquina 
